...
class addFood extends Component {
  ...
  validate() {
    ...
    if (!this.state.Price) {
      priceError = "Please fill this";
    }

    if (!this.state.Description) {
      desError = "Please fill this";
    }
    ...
    if (ratingError || nameError || desError || priceError) {
      this.setState({
        ratingError,
        nameError,
        desError,
  ...
  addFood = () => {
    ...
    if (isValid) {
      axios
        .post("http://localhost:3000/add/" + this.state.id, data)
        .then((response) => {
          console.log(response);
          toast.success("New flight has been added");
        });
    }
  };
    ...
    return (
      <div>
        ...
        <section class="get-in-touch">
          ...
          <button className="submit-btn" type="submit" onClick={this.addFood}>
            Submit
          </button>
        </section>
      </div>
