...
class Cart extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:3000/booking/show", this.state.config)
      .then((response) => {
    ...
    toast(
      <div style={{ margin: "20px;" }}>
        <p className="text-center">
          <i className="fa fa-shopping-cart" /> Confirm Flight?
        </p>

        <button className="btn btn-dark">
          <Link to="/checkOut">Yes</Link>
        </button>
  ...
  deleteBooking = (bid) => {
    axios
      .put(
        "http://localhost:3000/delete/booking/" + bid,
        this.state,
        this.state.config
      )
      .then((response) => {
        toast.success("Flight deleted from cart!");
  ...
  updateBooking = (pid) => {
    axios
      .put(
        "http://localhost:3000/updatebooking/" + pid,
        this.state,
        this.state.config
      )
      .then((response) => {
  ...
  plus = (value) => {
    ...
    axios
      .put(
        "http://localhost:3000/updatebooking/" + value.item._id,
        { Qty: quan },
        this.state.config
      )
      .then((response) => {
