...
class Cart extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:3000/bookingHotel", this.state.config)
      .then((response) => {
        if (response.data.data === null || response.data.data.length === 0) {
          this.setState({ noData: "You do not have any items in the cart" });
          alert("Your bookings is empty");
          //window.location.href = '/'
        } else {
          console.log("Booked items:::::", response.data);
          this.setState({
    ...
    toast(
      <div style={{ margin: "20px;" }}>
        <p className="text-center">
          <i className="fa fa-shopping-cart" /> Confirm Booking?
        </p>

        <button className="btn btn-dark">
          <Link to="/checkOutBook">Yes</Link>
        </button>
  ...
  deleteBooking = (bid) => {
    axios
      .put(
        "http://localhost:3000/delete/bookingHotel/" + bid,
        this.state,
        this.state.config
      )
      .then((response) => {
  ...
  updateBooking = (pid) => {
    axios
      .put(
        "http://localhost:3000/updatebookingHotel/" + pid,
        this.state,
        this.state.config
      )
      .then((response) => {
  ...
  plus = (value) => {
    ...
    axios
      .put(
        "http://localhost:3000/updatebookingHotel/" + value.item._id,
        { Qty: quan },
        this.state.config
      )
      .then((response) => {
        ...
        <div className="container main-section">
          <div className="row">
            <div className="col-lg-12 pb-2">
              <h4>Welcome to my bookings</h4>
              <h4 className="mt-3" style={{ fontSize: 20, color: "red" }}>
                  {this.state.noData}
                </h4>
            </div>
                  ...
                  {this.state.data.map((booking, i) => {
                    return (
                      <tr>
                        ...
                        <td>
                          <strong>{booking.qty * booking.item.Price}</strong>
                        </td>
                        <td className="actions" data-th="" width="10%;">
                          <button
                            className="btn btn-info btn-sm"
                            onClick={this.updateBooking.bind(
                              this,
                              booking.item._id
                            )}
                          >
                          ...
                          <button
                            ...
                            onClick={this.deleteBooking.bind(this, booking._id)}
                          >
