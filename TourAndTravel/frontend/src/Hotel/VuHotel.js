...
class Home extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:3000/room/show")
      .then((response) => {
    ...
    toast(
      <div style={{ margin: "20px;" }}>
        <p className="text-center">
          <i className="fa fa-shopping-cart" /> Are You Suer to Book?
        </p>
  ...
  post = (pid) => {
    ...
    if (token) {
      axios
        .post(
          "http://localhost:3000/bookingHotel/" + pid,
          this.state,
          this.state.config
        )
        .then((response) => {
          if (response.success === true) {
            toast(<Link to="/cart">Added to Bookings</Link>);
          }
        });
  ...
  bookmark(pid) {
    ...
    if (token) {
      axios
        .post(
          "http://localhost:3000/bookmark/" + pid,
          this.state,
          this.state.config
        )
        .then((response) => {
